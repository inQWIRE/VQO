...
Fixpoint appx_full_adder' (x:var) (n:nat) (b:nat) (size:nat) (y:var) :=
  match n with
  | 0 => (SKIP (x,0))
  | S m => (if n <=? b then (CU (y,m) (SR (size - n) x)) else (SKIP (x,m))); appx_full_adder' x m b size y
  end.
...
Fixpoint appx_adder' (x:var) (n:nat) (b:nat) (size:nat) (M: nat -> bool) :=
  ...
  | S m => appx_adder' x m b size M ;
           if n <=? b then (if M m then SR (size - n) x else SKIP (x,m)) else SKIP (x,m)
  end.
...
Fixpoint appx_sub' (x:var) (n:nat) (b:nat) (size:nat) (M: nat -> bool) :=
  ...
  | S m => appx_sub' x m b size M;
             (if n <=? b then (if M m then SRR (size - n) x else SKIP (x,m)) else SKIP (x,m))
  end.
